...
public class MustMatchRule extends Rule {
	...
	public MustMatchRule(String id, Pattern path, String variable, int operator, String value, Action ruleDefaultAction) {
		this.path = path;
	...
	public Action check(HttpServletRequest req,
		...
		if ( ! path.matcher(uri).matches() ) {
			...
			if ( variable.startsWith( REQUEST_PARAMETERS ) ) {
				...
				target = variable.substring(SESSION_ATTRIBUTES.length());
						...
						while(e.hasMoreElements()) {
							...
							if (p.matcher(attr).matches() ) {
								...
								if (o==null ) {
									log(request, "MustMatch rule failed (input variable does not exist or is null)");
									return this.ruleDefaultAction;
								}else if ( ! RuleUtil.testValue((String)o, value, operator) ) {
